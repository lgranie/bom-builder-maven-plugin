...
public class BuildBomMojo extends AbstractMojo {
	...
	private List<Dependency> getArtifactsAsDependencies() {
		return mavenProject.getArtifacts().stream().map(this::map).filter(this::isDependencyNotExcluded)
				.collect(Collectors.toList());
	}

	private List<Dependency> getDependencies() {
		return mavenProject.getDependencies().stream().map(Dependency::clone).filter(this::isDependencyNotExcluded)
				.collect(Collectors.toList());
	}
	...
	private List<Dependency> getDependencyManagementDependencies() {
		...
		return dependencyManagement.getDependencies().stream().map(Dependency::clone)
				.filter(this::isDependencyNotExcluded).collect(Collectors.toList());
	}
	...
	boolean isDependencyNotExcluded(Dependency dependency) {
		...
		for (DependencyExclusion exclusion : dependencyExclusions) {
			if (matchesExcludedDependency(dependency, exclusion)) {
				getLog().debug("Dependency " + dependency.getGroupId() + ":" + dependency.getArtifactId()
	...
	private Properties generateVersionProperties(List<Dependency> dependencies) {
		...
		dependencies.forEach(dependency -> {
			...
			if (versionProperties.getProperty(versionPropertyName) != null
				...
				versionPropertyName = VERSION_PROPERTY_PREFIX + dependency.getGroupId() + "."
